...
## Привязь к станции (item 13): дальше этого дрон от своей станции не уходит.
##
## Это НЕ запас хода, а радиус связи, и он вдвое короче самого полёта. Смысл в том,
## что дальность 30 — это про «долететь и вернуться», а не «улететь на другой край
## карты»: с привязью в 30 дрон уходил ровно туда, откуда уже не работал ни как
## разведчик станции, ни как заряд, и станция превращалась в кнопку «отправить дрон
## насовсем». С привязью 15 полный бак по-прежнему тратится за один подлёт, но
## тратится ВОКРУГ станции.
const DRONE_LEASH := 15
...
## Сколько тел боец может нести в руках. ОДНО (item 15: «it's still possible to pick up
## more than 1 corpse»). Второй труп поднять нельзя — ни кнопкой, ни подсветкой, ни
## намерением по сети: всё это спрашивает у одной этой константы.
##
## Раньше их было два, и труп-щит давал по +1 к защите за каждый — то есть носильщик
## получал +2 и становился крепче любого укрытия, продолжая при этом стрелять.
const CORPSE_CARRY_MAX := 1
